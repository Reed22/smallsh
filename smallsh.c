...
int main(int argc, char** argv){
    ...
    do {
        ...
        //Used for interruptions with singal handlers, restarts
        //do while loop if signal interrupts
        if(numChars == -1){
            clearerr(stdin);
        }
        else{
            ...
            //This works for every other command using fork() and execvp()
                ...
                //EINTR_set and EINTR_status is probably an improper way to print termination signals for SIGINT
                bool EINTR_set = false;
                int EINTR_status;
